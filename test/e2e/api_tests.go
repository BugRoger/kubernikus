	...
	s.waitForCluster(
		...
		func(k *models.Cluster, err error) bool {
			...
			if k.Status.Kluster.State == ClusterReady {
				log.Printf("Cluster %v is ready", *k.Name)
				...
				log.Printf("Cluster %v not ready. Still in %v", *k.Name, k.Status.Kluster.State)
			}
			return false
		},
	)
...
func (s *E2ETestSuite) TestUpdateCluster() {
	...
	err = s.waitForCluster(
		...
		func(k *models.Cluster, err error) bool {
			...
			s.testing.Errorf("Cluster %v was not updated or not all nodes are healthy and running", *k.Name)
			return false
		},
	)
	s.handleError(err)
